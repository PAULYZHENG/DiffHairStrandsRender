...
def render_depth(hair_path, camera_path, mask_path, output_path, output_with_seg_path):
    ...
    image = images.detach().cpu().numpy()[0].transpose((1, 2, 0))[:,:,0]
    ...
    #     mask = mask[:,:,0]
    # mask_bool = (mask>0.5)
    # np.save(output_with_seg_path, image*mask_bool)
...
if __name__ == '__main__':
    ...
    for item in tqdm(items):
        ...
        hair_path = root + 'test_results/' + name + '/' + item[:-3] + 'ply'
        camera_path = root + 'param/' + item[:-3] + 'npy'
        mask_path = root + 'seg/' + item[:-3] + 'png'
        output_path = root + 'test_results/rendered_depth/' + name + '/'  + item[:-3] + 'png'
        output_with_seg_path = root + 'test_results/rendered_depth_masked/' + name + '/'  + item[:-3] + 'png'

        os.makedirs(root + 'test_results/rendered_depth/' + name + '/', exist_ok=True)
        os.makedirs(root + 'test_results/rendered_depth_masked/' + name + '/', exist_ok=True)

        render_depth(hair_path, camera_path, mask_path, output_path, output_with_seg_path)

# CUDA_VISIBLE_DEVICES=1 screen python -m app.render_depth_from_prediction