...
class RenderOrien(nn.Module):
    def __init__(self, hair_path, camera_path, directional=True, width=256, deg=0):
        super(RenderOrien, self).__init__()
    ...
    def set_texture(self, verts):
        ...
        cur_orien[:,0] = -cur_orien[:,0]
        if self.directional:
            cur_orien[:,0] = cur_orien[:,0]/ 2.0 + 0.5
            cur_orien[:,1] = cur_orien[:,1]/ 2.0 + 0.5
        else:
    ...
    def forward(self):
        ...
        textures = self.set_texture(hair_vertices.squeeze()[:int(self.num_hairV/2),:])
